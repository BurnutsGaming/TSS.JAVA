 ...
 */
    public static int digestSize(TPM_ALG_ID alg) {
        return switch (alg.asEnum()) {
            case SHA1 -> 20;
            case SM3_256, SHA256 -> 32;
            case SHA384 -> 48;
            case SHA512 -> 64;
            default -> throw new RuntimeException("Unknown algorithm ID (not a hash?)");
        };
    }
/**
 * Hash data
 ...
 */
    ...
    public static boolean validateSignature(TPMT_PUBLIC _pubKey, byte[] _dataThatWasSigned, TPMU_SIGNATURE _signature) {
        ...
        if (_pubKey.parameters instanceof TPMS_RSA_PARMS rsaParms) {
...
/*
                ...
                return signerX.verifySignature(theRsaSig.sig);
*/
                ...
                return signer.verifySignature(theRsaSig.sig);
            }
            if (rsaParms.scheme instanceof TPMS_SIG_SCHEME_RSASSA scheme) {
        ...
        if (_pubKey.parameters instanceof TPMS_ECC_PARMS eccParms)
        {
    ...
    public static ECPublicKey decodeKey(byte[] encoded) throws InvalidKeySpecException, NoSuchAlgorithmException, NoSuchProviderException
    {
        ...
        java.security.spec.ECParameterSpec params2 = EC5Util.convertSpec(ellipticCurve, new ECParameterSpec(curve, params.getG(), params.getN(), params.getH(), params.getSeed()));
    ...
    public static Digest getDigest(TPM_ALG_ID alg) {
        return switch (alg.asEnum()) {
            ...
            case SM3_256 -> new SM3Digest();
            default -> throw new RuntimeException("No such digest");
        };
    ...
    static byte[] stringToLabel(String s) {
        return Helpers.concatenate(StandardCharsets.UTF_8.encode(s).array(), new byte[] { 0 });
    }
    ...
    {
        if (pub.parameters instanceof TPMS_RSA_PARMS rsaParms)
        {
    ...
    public static ECCKeyPair createECCKey(TPM_ECC_CURVE curveId, TPM_ALG_ID alg)
    {
        try
        {
            ...
            gen.initialize(new ECParameterSpec(ecSpec.getCurve(), ecSpec.getG(), ecSpec.getN(), ecSpec.getH(), ecSpec.getSeed()), random);
            ...
            BCECPrivateKey priv1 = (BCECPrivateKey)pairX.getPrivate();
            BCECPublicKey pub1 = (BCECPublicKey)pairX.getPublic();
            
            newKey.PublicKey = pub1.getQ();
            newKey.PrivateKey = priv1.getD();
    ...
    {
        return switch (curve.asEnum()) {
            case NIST_P256 -> "P-256";
            default -> throw new TpmException("Unsupported alg");
        };
    }
    
    static String ecTpmToBc(TPM_ALG_ID id)
    {
        return switch (id.asEnum()) {
            case ECDSA -> "ECDSA";
            case ECDH -> "ECDH";
            default -> throw new TpmException("Unsupported alg");
        };
